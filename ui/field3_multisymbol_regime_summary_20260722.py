...
def _resolve_current_candle_datetime(
    ...
    """
    Resolve the actual completed-candle datetime used by Field 3.

    Priority:
    1. Explicit datetime columns in ranking.
    2. Explicit datetime columns in evidence/state.
    3. Explicit Field 3 state datetime keys.

    IMPORTANT:
    This function must use the actual market candle datetime.
    It must NOT use datetime.now(), because that would create
    ...
    # ---------------------------------------------------------
    # 3. Search explicit state values
    # ---------------------------------------------------------
...
def _load_middle_regime_history(
    ...
    """
    Load persistent Middle Regime history.

    Persistent Parquet history is combined with the current
    ...
    # ---------------------------------------------------------
    # Current Streamlit session history
    # ---------------------------------------------------------
    ...
    if (
        isinstance(session_history, pd.DataFrame)
        and not session_history.empty
    ):
        frames.append(session_history)
...
def _save_middle_regime_history_snapshot(
    ...
    """
    Save the current Middle Regime ranking as a historical snapshot.

    One snapshot contains every symbol for one actual completed
    candle datetime.
    ...
    Duplicate Symbol + Datetime records are replaced.
    """
    ...
    current_datetime = _resolve_current_candle_datetime(
        ranking,
        state,
    )
    ...
    snapshot = middle_age.copy()

    # ---------------------------------------------------------
    # Every row belongs to this completed candle
    # ---------------------------------------------------------

    snapshot["Datetime"] = current_datetime

    # ---------------------------------------------------------
    # Load existing persistent + session history
    # ---------------------------------------------------------

    history = _load_middle_regime_history(
        state
    )
    ...
    # ---------------------------------------------------------
    # Remove duplicate Symbol + Datetime
    # ---------------------------------------------------------
...
def _find_datetime_column(frame: pd.DataFrame) -> str | None:
    """
    Find the datetime column used by historical Middle Regime data.
    """
    ...
    # look for a column containing date/time information.
...
def _prepare_middle_history(
    ...
    """
    Prepare historical Middle Regime records for the day/time finder.
    """
    ...
    datetime_column = _find_datetime_column(table)

    if datetime_column is None:
        return pd.DataFrame()

    table["_FinderDatetime"] = pd.to_datetime(
        table[datetime_column],
        errors="coerce",
    )

    table = table.dropna(
        subset=["_FinderDatetime"]
    ).copy()

    if table.empty:
        return table

    # Rebuild the Middle ranking fields if necessary.
    if MIDDLE_AGE_RANK_COLUMN not in table.columns:
        if CANDLE_AGE_COLUMN in table.columns:
    ...
    # Keep newest historical record first.
...
def render_middle_regime_finder(
    ...
    """
    Expandable historical Middle Regime finder.

    User selects a day and time, clicks Find, and the matching
    Middle Regime ranking is displayed with its own CSV download.
    """

    st.markdown("#### 🔎 Middle Regime Historical Finder")
    ...
    if not isinstance(history, pd.DataFrame) or history.empty:
        st.info(
            "No historical Middle Regime records are available yet. "
            "Run Field 3 calculations so completed-candle snapshots "
            "can be saved."
        )
        return
    ...
    if history.empty:
        st.warning(
            "Historical Middle Regime data exists, but no valid "
            "date/time values were found."
        )
        return

    datetime_values = history["_FinderDatetime"]

    min_datetime = datetime_values.min()
    max_datetime = datetime_values.max()
    ...
    available_days = sorted(
        datetime_values.dt.date.dropna().unique()
    )

    if not available_days:
        st.warning("No valid historical dates are available.")
        return
    ...
    selected_day_rows = history.loc[
        datetime_values.dt.date == selected_day
    ].copy()

    if selected_day_rows.empty:
        st.warning(
            f"No Middle Regime records were found for {selected_day}."
        )
        return

    available_times = (
        selected_day_rows["_FinderDatetime"]
        .dt.strftime("%H:%M")
        .drop_duplicates()
        .tolist()
    )
    ...
    if not available_times:
        st.warning(
            "No valid times are available for the selected day."
        )
        return
    ...
    find_clicked = st.button(
        "🔎 Find Middle Regime Ranking",
        key="find_middle_regime_history",
    ...
    target_time = pd.to_datetime(
        f"{selected_day} {selected_time}",
        errors="coerce",
    )

    if pd.isna(target_time):
        st.error("The selected day/time is invalid.")
        return

    # ---------------------------------------------------------
    # Exact datetime first
    # ---------------------------------------------------------

    exact = history.loc[
        history["_FinderDatetime"] == target_time
    ].copy()

    # ---------------------------------------------------------
    # If there isn't an exact timestamp, find the latest
    # snapshot at or before the selected time.
    # ---------------------------------------------------------

    if exact.empty:

        # Only search within the selected day.
        same_day = history.loc[
            history["_FinderDatetime"].dt.date == selected_day
        ].copy()

        before = same_day.loc[
            same_day["_FinderDatetime"] <= target_time
        ].copy()

        if before.empty:
            st.warning(
                "No Middle Regime ranking exists at or before "
                f"{selected_time} on {selected_day}."
            )
            return

        nearest_time = before["_FinderDatetime"].max()

        exact = before.loc[
            before["_FinderDatetime"] == nearest_time
        ].copy()

        st.info(
            "No exact candle timestamp was found. "
            f"Showing the latest available snapshot on "
            f"{selected_day}: "
            f"{nearest_time.strftime('%Y-%m-%d %H:%M')}"
        )
...
# ---------------------------------------------------------
    ...
    found_datetime = pd.to_datetime(
        exact["_FinderDatetime"].iloc[0]
    )

    st.success(
        "Middle Regime ranking found for "
        f"{found_datetime.strftime('%Y-%m-%d %H:%M')}"
    )

    st.caption(
        f"Selected snapshot contains {len(result)} symbol(s)."
    )

    # ---------------------------------------------------------
    # CSV download for THIS historical result
    # ---------------------------------------------------------

    historical_csv = result.to_csv(
        index=False,
        encoding="utf-8",
    )

    st.download_button(
        label="📥 Download This Middle Regime Ranking CSV",
        data=historical_csv,
        file_name=(
            "middle_regime_"
            f"{found_datetime.strftime('%Y%m%d_%H%M')}.csv"
        ),
        mime="text/csv",
        key=(
            "download_middle_regime_history_"
            f"{found_datetime.strftime('%Y%m%d_%H%M')}"
        ),
        use_container_width=True,
    )
    ...
    render_responsive_records(
        ...
        desktop_height=min(
            620,
            100 + 38 * len(display_result),
        ),
        full_table_label=(
            "Full Historical Middle Regime Ranking"
        ),
...
def render_multisymbol_regime_summary(st: Any, state: MutableMapping[str, Any]) -> None:
    ...
    st.write(
        "DEBUG ranking datetime columns:",
        [
            column
            ...
            if (
                ...
                or "stamp" in str(column).lower()
            )
        ],
    )

    st.write(
        "DEBUG state datetime values:",
        {
            key: state.get(key)
            for key in (
                "field3_current_candle_datetime",
    ...
    _save_middle_regime_history_snapshot(
        state,
        middle_age,
        ranking,
    )
   
    history_status = _load_middle_regime_history(
        state
    )
    ...
    if isinstance(history_status, pd.DataFrame) and not history_status.empty:
        ...
        if not history_dates.empty:
            ...
            st.caption(
                "Middle Regime history: "
                f"{len(history_status)} records · "
                f"{history_dates.nunique()} snapshot(s) · "
                f"{history_dates.min().strftime('%Y-%m-%d %H:%M')} "
                "→ "
                f"{history_dates.max().strftime('%Y-%m-%d %H:%M')}"
            )
    ...
    if middle_age.empty:
        ...
        st.info(
            "Middle regime-age values are not available yet "
            "for the completed universe."
        )
    else:

        # ---------------------------------------------------------
        # CURRENT Middle Regime CSV
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # HISTORICAL DAY/TIME FINDER
        # ---------------------------------------------------------

        with st.expander(
            "🔎 Open Middle Regime Day/Time Finder",
            expanded=False,
        ):
        ...
        st.caption(
            ...
            "blue whole row = Middle Regime Bias matches "
            "Higher Standard Regime Bias. "
            "Top 5 and Last 5 row bands are also shown."
        )

        # ---------------------------------------------------------
        # CURRENT Middle Regime table
        # ---------------------------------------------------------
